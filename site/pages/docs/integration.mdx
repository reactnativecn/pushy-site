...
其中`checkUpdate`方法可以用来手动触发更新检查。检查后会返回[`updateInfo`](api#async-function-checkupdate)（注意在 v10.26.0 版本之前，`checkUpdate`方法本身没有返回值，只能从`usePushy()`返回的`updateInfo`中获取），有三种情况：
