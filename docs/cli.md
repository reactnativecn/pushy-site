> For AI agents: the complete documentation index is available at /llms.txt, the full documentation bundle is available at /llms-full.txt, and this page is available as Markdown at /docs/cli.md.
...
从 v2.22.0 版本开始，Hermes 工程默认启用 **delta 模式**（`hermesc -base-bytecode`）：打包时以同一应用上一版热更的 HBC 为 base 编译，让 Hermes 字符串表跨版本保持稳定，热更 patch 体积通常缩小 5～30 倍。整个过程对客户端透明，产物仍是标准 HBC；任何一步失败都会自动退回普通编译，不会阻塞发版。相关参数：

- hermesBase: `auto`（默认，向服务端查询本应用的 base 并按 sha256 校验）| `none`（关闭）| 本地文件路径（`.hbc` / `.ppk` / `.apk` / `.ipa`，例如用商店包的 bundle 作为 base）
- verifyHermesBase: 默认 `true`，base 编译后再做一次普通编译并比对两份反汇编，不等价则放弃 base；大 bundle 可传 `false` 换时间
- cacheMaxMb: 本地 bundle 缓存上限（默认 500 MB / 20 个文件），缓存位于 `.pushy/cache/<sha256>`，也可用环境变量 `PUSHY_CACHE_DIR` / `PUSHY_CACHE_MAX_MB` 调整

注意：只有包含上游 delta 模式修复的 hermesc 才会启用（React Native 自带的经典 hermesc，或 `hermes-compiler` ≥ 250829098）；升级 React Native 导致 HBC 版本变化时会自动开始新的链，首个版本不带 base。另外从 v2.22.0 起 hermesc 总是以 `-output-source-map` 调用，字节码不再包含 debug info（与 RN release 构建一致，体积减少 15%～40%），Hermes 自己的 `.map` 保留在 `.pushy/intermedia/<platform>/` 下以便事后符号化崩溃堆栈。
