...
- **🎯 超小更新包**：基于 bsdiff/hdiff 算法的增量更新，通常只需**几十至几百 KB**（相比其他全量更新服务的几十 MB，节省流量 99%+），并针对 Hermes 字节码做了**独家深度优化**（见下方实测数据）
...
Hermes 字节码中充满"偏移量表"——JS 一处小改动会让其后所有偏移整体位移，普通二进制 diff 会因此把补丁体积急剧放大。Pushy 独家引入了 **HBC（Hermes 字节码）结构感知的可逆变换**，从根源上消除这种放大。
...
| --------------- | ------ | ------------ | --------------------- |
| 修改一行文案          | 1.9 MB | 93.7 KB      | **63.5 KB**（再省 32%）   |
| 新增小功能（约 60 行）   | 1.9 MB | 411.6 KB     | **285.5 KB**（再省 31%）  |
| 新增中等功能（约 300 行） | 1.9 MB | 551.6 KB     | **398.4 KB**（再省 28%）  |

- 相比全量更新节省 **95% 以上**的流量，相比传统 bsdiff 增量**再省约 30%**
- **改动越小、收益越大**——恰好是热修复中最高频的场景
- 安全兜底：变换前对字节码结构做全量校验，任何不匹配自动回退到普通增量路径；Hermes 版本升级时**客户端零改动**自动兼容
