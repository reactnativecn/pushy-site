...
:::warning
`REQUEST_INSTALL_PACKAGES` 属于敏感权限，不应添加到不需要应用内 APK 安装的渠道包。Google Play 不允许使用该权限实现应用自更新；Google Play 渠道应移除此权限，并改为跳转应用商店或使用 Google Play 的应用内更新机制。移除该权限只会禁用 APK 整包安装，不影响 Pushy 的热更新功能。
:::
